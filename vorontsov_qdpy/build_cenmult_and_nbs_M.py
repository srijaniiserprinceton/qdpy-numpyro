...
def getnt4cenmult(CNM_AND_NBS, GVARS, return_shaped=False):
    ...
    ell0 = CNM_AND_NBS.nl_nbs[0,1]  # extracting the cenmult ell0
    ...
    M_couplings_nl = np.zeros((num_nbs, num_nbs, 2))
    ...
    for i, ellp in enumerate(CNM_AND_NBS.nl_nbs[:,1]):
        for j, ellp_ in enumerate(CNM_AND_NBS.nl_nbs[:,1]):
            ...
            M_coupling_nl[i,j,:] = np.array([ell1, ell2])
    ...
    nl_neighbours_M = np.reshape(M_couplings_nl, (num_nbs * num_nbs, 2))
    nl_neighbours_M_idx = nl_idx_vec(nl_neighbours)
    omega_neighbours_M = get_omega_neighbors(nl_neighbours_idx)
    
    if return_shaped:
        # reshaping to be used in the loop in sparse_precompute_M
        nl_neighbours_M = np.reshape(nl_neighbours, (num_nbs, num_nbs, 2))
        nl_neighbouts_M_idx = np.reshape(nl_neighbours_M_idx, (num_nbs, num_nbs))
        omega_neighbours_M = np.reshape(omega_neighbours, (num_nbs, num_nbs))
