...
python dpy_jax/mode_lister.py --nmin $NMIN --nmax $NMAX --lmin $LMIN --lmax $LMAX >.mlist.out 2>.mlist.err
echo "       -- `tail -1 .mlist.out`"

echo "[ 2. ] Creating Ritzwoller Lavely polynomials ..."
python dpy_jax/precompute_ritzlavely.py >.rl.out 2>.rl.err
echo "       -- `tail -1 .rl.out`"

echo "[ 3. ] Generating synthetic eigenvalues ..."
python dpy_jax/generate_synthetic_eigvals.py --load_mults 1 --knot_num $KNOT_NUM --rth $RTH
echo "       -- DONE"
