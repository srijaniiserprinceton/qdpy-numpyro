...
for dayind in range(dayind_min, dayind_max+1):
    ...
    wsr_hybrid = np.load(f'wsr_hybrid_fit_{day}.npy')
...
plt.figure()
plt.pcolormesh(yy, tt * 180./np.pi, Omega_hybrid_timearr_rel, cmap='jet')
plt.colorbar()
plt.savefig('Omega_hybrid_timearr_rel.pdf')


plt.figure()
plt.pcolormesh(yy, tt * 180./np.pi,
               Omega_hybrid_timearr_rel - Omega_dpt_timearr_rel, cmap='jet')
plt.colorbar()
plt.savefig('Omega_hybrid_rel_dpt.pdf')
