...
nl_arr = np.load('nl.0_30.0_300.1216.36.npy')

smax = 9

# loading the acoeff_HMI array
ac = np.load('acoeffs_MDI.mdi.72d.1216.36.npy')[:,:smax+1:2].T
# ac = np.reshape(ac, (5, -1), 'F')

# loading the acoeff_HMI sigma array
ac_sigma = np.load('acoeffs_sigma_MDI.mdi.72d.1216.36.npy')[:,:smax+1:2].T
# ac_sigma = np.reshape(ac_sigma, (5, -1), 'F')

# choosing from index 33 of nl_arr which is supposed to be before n = 5
# SO, consistent with nmin >= 5
ac = ac[:, 33:]
ac_sigma = ac_sigma[:, 33:]
nl_arr = nl_arr[33:,:]
