...
os.system(f"python {run_newton_py} --read_hess 1 --instrument {instr} " +
          f"--mu {mu_limits[0]} --batch_run 1 --batch_rundir {PARGS.rundir} " +
          f">{tempout} 2>{temperr}")
os.system(f"python {run_newton_py} --read_hess 1 --instrument {instr} " +
          f"--mu {mu_limits[1]} --batch_run 1 --batch_rundir {PARGS.rundir} " +
          f">{tempout} 2>{temperr}")
...
for i in range(maxiter):
    ...
    muval = 10**log_muval
    os.system(f"python {run_newton_py} --read_hess 1 --instrument {instr} " +
              f"--mu {muval} --batch_run 1 --batch_rundir {PARGS.rundir} " +
              f">{tempout} 2>{temperr}")

    muval1 = 10**(0.5*(np.log10(leftmu) + log_muval))
    muval2 = 10**(0.5*(np.log10(rightmu) + log_muval))
    os.system(f"python {run_newton_py} --read_hess 1 --instrument {instr} " +
              f"--mu {muval1} --batch_run 1 --batch_rundir {PARGS.rundir} " +
              f">{tempout} 2>{temperr}")
    os.system(f"python {run_newton_py} --read_hess 1 --instrument {instr} " +
              f"--mu {muval2} --batch_run 1 --batch_rundir {PARGS.rundir} " +
              f">{tempout} 2>{temperr}")
    ...
    mf1 = compute_misfit(val0, val1)
    mf2 = compute_misfit(val0, val2)
    print(f"muval = {muval1:.5e}; misfit = {mf1:.5e}")
